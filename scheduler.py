    ...
    try:
        ...
        if today.weekday() == 0: # 0 = Lunes
            if current_time < time(12, 0):
                ...
                logger.info("ℹ️  Es lunes tarde, periodo de recuperación finalizado.")
                return
            ...
            logger.info("ℹ️  Día entre semana, usando rango actual de lunes a hoy.")
            days_since_monday = today.weekday()
            week_start = today - timedelta(days=days_since_monday)
            week_end = today
...
# ========================
# VERIFICACIÓN MENSUAL (CON RECUPERACIÓN) - FUNCIÓN ORIGINAL
# ========================
    ...
    try:
        ...
        if today.day == 1 and current_time < time(12, 0):
            ...
            if month_end != today and not is_recovery:
                logger.info("ℹ️  No es fin de mes, no anuncia mensual.")
                return
        ...
        # Asegúrate de que el contexto tiene el bot
        from telegram.ext import Application
        app = Application.builder().token(config.TOKEN).build()
        # O usa el bot del contexto si está disponible de otra manera
        ...
        # Aquí asumimos que se llama desde el wrapper con el contexto correcto
        import asyncio
